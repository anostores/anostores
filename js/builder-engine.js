/* ==========================================
   ...
   /* 🌟 دالة الإشعارات الشيك والاحترافية */
   ...
   /* ربط الأحداث للتفاعل الفوري مع المعاينة */
   function bindEvents() {
     const inputIds = [
   ...
   /* حفظ المنتج ونشره مباشرة في Supabase */
   async function saveProductData() {
     ...
     try {
       ...
       if (currentEditProductId) {
         const res = await _supabase.from('products').update(payload).eq('id', currentEditProductId);
         error = res.error;
       } else {
       ...
       showChicNotification('❌ حدث خطأ أثناء الحفظ: ' + err.message, 'error');
     } finally {
       btn.disabled = false;
