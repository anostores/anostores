/* ==========================================================================
   ...
   Full Features: Dynamic Category Analytics, Maintenance Mode Toggle, 
   Detailed Visitor Analytics, Bot Direct Support Board, Slider Live Preview, 
   Strict Permissions, Dynamic WhatsApp Contacts & Direct Reply Board
   ...
   document.addEventListener("DOMContentLoaded", async () => {
     ...
     loadBotSupportRequestsBoard();
     loadDetailedAnalyticsTable();
     initFooterPageEditor();
   ...
   async function loadCategoryAnalytics() {
     const categoryProgressBar = document.getElementById("top-category-progress");
     ...
     try {
       ...
       if (!orders || orders.length === 0) {
         ...
         categoryProgressBar.innerText = "لا توجد طلبات بعد";
         categoryProgressBar.className = "progress-bar bg-secondary fw-bold fs-9";
         return;
       }
       ...
       categoryProgressBar.setAttribute("aria-valuenow", percentage);
       categoryProgressBar.innerText = `${displayCategoryName} (${percentage}%)`;
   ...
   /* 2.3 إدارة ومتابعة وضع صيانة المتجر التفاعلي */
   async function loadMaintenanceStatus() {
     const btn = document.getElementById("toggle-maintenance-btn");
     if (!btn) return;
   
     try {
       const { data } = await _supabase.from("store_settings").select("value").eq("key", "maintenance_mode").maybeSingle();
       const isEnabled = data?.value?.is_enabled || false;
   
       if (isEnabled) {
         btn.className = "btn btn-sm btn-warning text-dark fw-bold w-100 rounded-pill";
         btn.innerHTML = '<i class="fa-solid fa-lock me-1"></i> وضع الصيانة: مفعّل الآن';
       } else {
         btn.className = "btn btn-sm btn-outline-success fw-bold w-100 rounded-pill";
         btn.innerHTML = '<i class="fa-solid fa-globe me-1"></i> وضع الصيانة: معطل (المتجر يعمل)';
       }
     } catch (err) {
   ...
   async function toggleMaintenanceMode() {
     const hasPerm = await checkPermissionOrPrompt('manage_admins', 'تفعيل وضع الصيانة');
     if (!hasPerm) return;
   
     try {
       const { data } = await _supabase.from("store_settings").select("value").eq("key", "maintenance_mode").maybeSingle();
       const currentState = data?.value?.is_enabled || false;
       ...
       await _supabase.from("store_settings").upsert({
         ...
         value: {
           ...
           message: "نعمل حالياً على تحديث وتطوير المتجر وسنعود إليكم فوراً!"
         },
         updated_at: new Date()
       });
       ...
       Swal.fire({
         icon: 'success',
         title: newState ? 'تم تفعيل وضع الصيانة!' : 'تم تعطيل وضع الصيانة والعودة للعمل!',
         text: newState ? 'تم توجيه زوار المتجر تلقائياً لصفحة الصيانة.' : 'المتجر يعمل بشكل طبيعي الآن للجمهور.',
         confirmButtonColor: '#e60023'
       });
   
       loadMaintenanceStatus();
     } catch (err) {
       Swal.fire({ icon: 'error', title: 'خطأ في التعديل', confirmButtonColor: '#e60023' });
     }
   }
   ...
   /* 12. استعراض طلبات التواصل المباشر من البوت الذكي */
   async function loadBotSupportRequestsBoard() {
     const tbody = document.getElementById("bot-requests-tbody");
     if (!tbody) return;
   
     try {
       const { data: reqs, error } = await _supabase.from("bot_support_requests").select("*").order("created_at", { ascending: false });
       if (error || !reqs || reqs.length === 0) {
         tbody.innerHTML = '<tr><td colspan="5" class="text-muted py-4">لا توجد طلبات تواصل من البوت حتى الآن.</td></tr>';
         return;
       }
       ...
       tbody.innerHTML = reqs.map(r => {
         ...
         return `
           <tr>
             <td class="fw-bold">${r.user_name || 'زائر المتجر'}</td>
             <td class="fw-bold text-danger">${r.phone_number}</td>
             <td class="text-start small" style="max-width: 250px;">${r.user_message || 'طلب تواصل من البوت'}</td>
             <td class="text-muted fs-9">${new Date(r.created_at).toLocaleDateString('ar-EG')}</td>
             <td>
               <a href="${waLink}" target="_blank" class="btn btn-sm btn-success py-0 px-2 fw-bold">💬 تواصل واتساب</a>
             </td>
           </tr>
         `;
       }).join('');
     } catch (err) {
       tbody.innerHTML = '<tr><td colspan="5" class="text-muted py-4">خطأ في التحميل.</td></tr>';
     }
   }
   
   /* 13. استعراض جدول التحليلات التفصيلية وحركات الزوار */
   async function loadDetailedAnalyticsTable() {
     const tbody = document.getElementById("detailed-analytics-tbody");
     if (!tbody) return;
   
     try {
       const { data: logs, error } = await _supabase.from("site_analytics").select("*").order("created_at", { ascending: false }).limit(50);
       if (error || !logs || logs.length === 0) {
         tbody.innerHTML = '<tr><td colspan="4" class="text-muted py-4">لا يوجد سجل زيارات مسجل.</td></tr>';
         return;
       }
   
       tbody.innerHTML = logs.map(l => `
         <tr>
           <td class="fw-bold text-dark text-start">${l.page_path || 'الصفحة الرئيسية'}</td>
           <td><span class="badge bg-light text-dark border">زيارة مباشرة</span></td>
           <td class="text-muted fs-9">${l.user_id ? 'عميل مسجل' : 'زائر جديد'}</td>
           <td class="text-muted fs-9">${new Date(l.created_at).toLocaleString('ar-EG')}</td>
         </tr>
       `).join('');
     } catch (err) {
       tbody.innerHTML = '<tr><td colspan="4" class="text-muted py-4">خطأ في تحميل سجل الزيارات.</td></tr>';
     }
   }
   
   /* 14. محرر صفحات الفوتر الديناميكية */
   function initFooterPageEditor() {
     const select = document.getElementById("footer-page-select");
     if (!select) return;
   
     select.addEventListener("change", () => loadFooterPageData(select.value));
     loadFooterPageData(select.value);
   }
   
   async function loadFooterPageData(pageKey) {
     const titleInput = document.getElementById("footer-page-title");
     const contentInput = document.getElementById("footer-page-content");
     const fontSelect = document.getElementById("footer-font-size");
     const alignSelect = document.getElementById("footer-text-align");
   
     if (!titleInput || !contentInput) return;
   
     try {
       const { data } = await _supabase.from("store_settings").select("value").eq("key", `page_${pageKey}`).maybeSingle();
       const pData = data?.value || {};
   
       titleInput.value = pData.title || "";
       contentInput.value = pData.content || "";
       if (fontSelect) fontSelect.value = pData.font_size || "16px";
       if (alignSelect) alignSelect.value = pData.text_align || "right";
   ...
   async function saveFooterPageData() {
     const hasPerm = await checkPermissionOrPrompt('manage_admins', 'تعديل صفحات الفوتر الديناميكية');
     if (!hasPerm) return;
   
     const pageKey = document.getElementById("footer-page-select")?.value;
     const title = document.getElementById("footer-page-title")?.value.trim();
     const content = document.getElementById("footer-page-content")?.value.trim();
     const fontSize = document.getElementById("footer-font-size")?.value || "16px";
     const textAlign = document.getElementById("footer-text-align")?.value || "right";
   
     if (!title || !content) {
       Swal.fire({ icon: 'warning', title: 'يرجى كتابة العنوان والمحتوى!', confirmButtonColor: '#e60023' });
       return;
     }
     ...
     try {
       await _supabase.from("store_settings").upsert({
         key: `page_${pageKey}`,
         value: { title, content, font_size: fontSize, text_align: textAlign },
         updated_at: new Date()
       });
   
       Swal.fire({ icon: 'success', title: 'تم حفظ ونشر محتوى الصفحة بنجاح!', confirmButtonColor: '#e60023' });
     } catch (err) {
       Swal.fire({ icon: 'error', title: 'حدث خطأ أثناء الحفظ', confirmButtonColor: '#e60023' });
     }
   }