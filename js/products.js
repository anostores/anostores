/* ==========================================================================
   ...
   Guaranteed Hybrid Routing Support for Live Server, GitHub Pages, Vercel & anostores.com
   ========================================================================== */
    ...
    if (isProductsPage) {
      ...
      if (resetBtn) {
        resetBtn.addEventListener("click", () => {
          ...
          const resetPath = isLocalOrGithub ? "products.html" : "/products";
  ...
  /* ================= 0. جلب وبناء السلايدر الديناميكي ================= */
  async function loadDynamicHeroSlider() {
    ...
    try {
      ...
      sliderInner.innerHTML = slides.map((slide, index) => {
        ...
        const targetLink = slide.link_url 
          ? (isLocalOrGithub && !slide.link_url.includes('.html') && !slide.link_url.startsWith('http') ? 'products.html' : slide.link_url) 
          : (isLocalOrGithub ? "products.html" : "/products");
        ...
        return `
          <div class="carousel-item ${activeClass} h-100" data-bs-interval="4000">
            <a href="${targetLink}" class="hero-slide-item d-block text-decoration-none h-100">
              <img src="${bgImg}" alt="${slide.title || 'عرض خاص'}" loading="lazy" class="d-block w-100 h-100 object-fit-cover">
            </a>
          </div>
        `;
      }).join("");
  ...
  /* ================= 3. إنشاء كارت المنتج القابل للضغط بالـ Slug ومسارات خالية من الأخطاء ================= */
