/* ==========================================================================
  ...
  /* ================= 0. جلب وبناء السلايدر الديناميكي (صور صافية ورابط مباشر) ================= */
    ...
    try {
      ...
      sliderInner.innerHTML = slides.map((slide, index) => {
        ...
        const targetLink = slide.link_url 
          ? (isLocalServer && !slide.link_url.includes('.html') && !slide.link_url.startsWith('http') ? 'products.html' : slide.link_url) 
          : (isLocalServer ? "products.html" : "/products");
  
        return `
          <div class="carousel-item ${activeClass}" data-bs-interval="4000">
            <a href="${targetLink}" class="hero-slide-item d-block text-decoration-none">
              <img src="${bgImg}" alt="${slide.title || 'عرض خاص'}" loading="lazy">
            </a>
          </div>
        `;
      }).join("");
