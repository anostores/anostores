/* ==========================================================================
   ...
   Hybrid Slug & ID Query Engine with Clean URLs
   ========================================================================== */
   ...
   document.addEventListener("DOMContentLoaded", async () => {
     // استخراج الـ Slug من المسار المباشر النظيف أو من معلمات URL
     const pathSegments = window.location.pathname.split("/").filter(Boolean);
     const urlParams = new URLSearchParams(window.location.search);
     
     // إذا كان المسار يحتوي على Slug بعد /products/
     let productIdentifier = urlParams.get("id");
     if (!productIdentifier && pathSegments.length > 1 && pathSegments[0] === "products") {
       productIdentifier = decodeURIComponent(pathSegments[1]);
     }
   ...
   /* 1. جلب بيانات المنتج من Supabase (دعم مزدوج بـ Slug أو ID) */
   async function loadProductDetails(identifier) {
     ...
     try {
       ...
       if (slugData) {
         ...
         // 2. تجربة البحث بالـ ID إذا لم نجد نتائج للـ Slug
