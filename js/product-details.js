...
function buyNowHandler() {
  ...
  window.location.href = isLocalOrGithub ? "cart" : "/cart";
}
...
async function attemptAddReview(productId) {
  let userName = localStorage.getItem("ano_user_name");
  let userId = null;
  ...
  if (typeof _supabase !== "undefined") {
    ...
    if (session && session.user) {
      ...
      userName = session.user.user_metadata?.full_name || userName || session.user.email.split('@')[0];
    }
  }
  ...
  if (!userName) {
    Swal.fire({
      ...
      text: 'تسجيل الدخول مطلوب لإضافة تقييمك وحفظ بياناتك بالمتجر.',
