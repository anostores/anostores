/* ==========================================
       ...
       tbody.innerHTML = `
         <tr>
           <td colspan="5" class="text-center py-5 text-muted">
             ...
             <a href="/" class="btn btn-sm btn-danger rounded-pill mt-2">تصفح المنتجات والتسوق الآن</a>
           </td>
         </tr>
       `;
       ...
       if (method === "branch_pickup") {
         ...
         switch (method) {
           case "instapay":
             ...
             instructionsText.innerText = "يرجى التحويل على معرف انستا باي: anostore@instapay أو الرقم 01000000000.";
             break;
           case "vodafone_cash":
             instructionsTitle.innerText = "محفظة فودافون كاش للتحويل:";
             instructionsText.innerText = "يرجى تحويل المبلغ إلى الرقم (01000000000).";
             break;
           case "etisalat_cash":
             instructionsTitle.innerText = "محفظة اتصالات كاش للتحويل:";
             instructionsText.innerText = "يرجى تحويل المبلغ إلى الرقم (01100000000).";
             break;
           case "fawry":
   ...
   async function handleCheckoutSubmit(e) {
     ...
     let clientEmail = "";
     const { data: { session } } = await _supabase.auth.getSession();
     if (session && session.user) clientEmail = session.user.email;
     ...
     try {
       ...
       Swal.fire({
         ...
         window.location.href = res.isConfirmed ? "/profile" : "/";
       });
   
     } catch (err) {
       console.error("Checkout Error:", err);
       Swal.fire({ icon: 'error', title: 'فشل إرسال الطلب', text: 'تأكد من إعدادات الجدول وقاعدة البيانات.', confirmButtonColor: '#e60023' });
     } finally {
