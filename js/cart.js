...
function loadCartItems() {
  ...
  const homePath = isLocalOrGithub ? "index" : "/";
      ...
      <tr>
        <td>
          <div class="d-flex align-items-center gap-2">
            <img src="${item.image || 'https://via.placeholder.com/50'}" class="no-open-tab" ondragstart="return false;" style="width: 45px; height: 45px; object-fit: contain;">
...
async function handleCheckoutSubmit(e) {
  ...
  try {
    ...
    Swal.fire({
      ...
      background: '#ffffff',
      customClass: {
        popup: 'rounded-4 shadow-lg border border-2 border-danger'
      },
      html: `
        <div class="text-center py-2">
          <div class="mb-3 position-relative display-1">
            🎉 📦 ✨
          </div>
          <h2 class="fw-bold text-dark mb-2">🎉 ألف مبروك! تم إتمام طلبك بنجاح</h2>
          <p class="text-secondary small mb-4 fs-6">شكراً لثقتك الغالية واختيارك الشراء من <strong>ANO Store</strong></p>
          
          <div class="p-4 bg-light rounded-4 text-start border border-danger border-opacity-25 mb-4 shadow-sm">
            <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
              <span class="text-muted fw-bold small"><i class="fa-solid fa-receipt me-1 text-danger"></i> حالة الطلب:</span>
              <span class="badge bg-success px-3 py-2">قيد المراجعة والتجهيز 🚚</span>
            </div>
            <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
              <span class="text-muted fw-bold small">طريقة الدفع المختارة:</span>
              <span class="badge bg-dark px-2 py-1">${paymentMethod === 'branch_pickup' ? 'استلام واستلام من الفرع' : paymentMethod}</span>
            </div>
            ${selectedBranch ? `<div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2"><span class="text-muted fw-bold small">موقع الفرع:</span><strong class="text-dark">${selectedBranch}</strong></div>` : ''}
            <div class="d-flex justify-content-between align-items-center pt-1">
              <span class="text-muted fw-bold fs-6">المبلغ الإجمالي المدفوع:</span>
              <strong class="text-danger fs-4">${totalAmount.toLocaleString()} ج.م</strong>
            </div>
          </div>

          <p class="text-muted fs-9 m-0">يمكنك مراجعة تفاصيل الشحنة وتتبع مسار التوصيل فوراً من ملفك الشخصي.</p>
        </div>
      `,
      showCancelButton: true,
      confirmButtonColor: '#e60023',
      cancelButtonColor: '#0d0d0d',
      confirmButtonText: '<i class="fa-solid fa-truck-fast me-1"></i> تتبع شحنتك في حسابي',
      cancelButtonText: '<i class="fa-solid fa-house me-1"></i> العودة للرئيسية'
    }).then((res) => {
