/* ==========================================
   ...
   With Strict Block Check, Direct Profiles Insertion & Clean Admin Protection
   ========================================== */
   ...
   document.addEventListener("DOMContentLoaded", async () => {
    ...
    // فحص حماية شاشة الأدمن فور التحميل
    await checkAdminPageProtection();
  
    // 1. تسجيل الدخول بـ Supabase Auth مع فحص الحظر والدور
    if (loginForm) {
      loginForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          // 1.2 الفحص الصارم لحالة حظر الحساب وجلب دور المستخدم
          let isBlocked = false;
          let userRole = 'user';
          ...
          if (profile) {
            ...
            // فحص احتياطي بجدول user_profiles
    ...
    // 2. إنشاء حساب جديد مع الإدراج المباشر بجدول profiles وإرسال رابط التفعيل
    if (regForm) {
      regForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          // إدراج مباشر فوري في جدول profiles للعملاء الجدد
          if (data && data.user) {
  ...
  /**
   * دالة حماية صفحات الأدمن من أي محاولة دخول لغير المصرح لهم
   */
  async function checkAdminPageProtection() {
    ...
    try {
      ...
      if (!session || !session.user) {
        window.location.href = isLocalServer ? "login.html?unauthorized=admin" : "/login?unauthorized=admin";
        return;
      }
