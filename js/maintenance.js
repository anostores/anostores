...
async function autoFillMaintenanceUser() {
  ...
  const { data: { session } } = await _supabase.auth.getSession();
  if (session && session.user) {
    const { data: profiles } = await _supabase.from("profiles").select("*").eq("id", session.user.id);
    ...
    if (phoneInput) phoneInput.value = profile?.phone_number || session.user.user_metadata?.phone || "";
  }
}
