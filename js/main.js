...
function renderHeroSliderHtml(slides) {
  ...
  sliderInner.innerHTML = slides.map((slide, index) => {
    ...
    if (slide.image_url) {
      return `
        <div class="carousel-item ${activeClass} h-100" data-bs-interval="4000">
          <a href="${slideLink}" class="hero-slide-item h-100 d-block">
            <img src="${slide.image_url}" class="d-block w-100 h-100 object-fit-cover no-open-tab" ondragstart="return false;" alt="${slide.title || 'ANO Store Slide'}">
          </a>
        </div>
      `;
    } else {
      return `
        <div class="carousel-item ${activeClass} h-100" data-bs-interval="4000">
          <a href="${slideLink}" class="hero-slide-item h-100 d-flex align-items-center justify-content-center bg-black text-white text-decoration-none p-4 text-center">
            <div>
              <h3 class="fw-bold text-danger mb-2">${slide.title || 'عروض أنو ستور الحصرية'}</h3>
              <p class="text-secondary small m-0">تصفح أحدث الموبايلات والعطور بضمان معتمد</p>
            </div>
          </a>
        </div>
      `;
...
function createProductCard(product) {
  ...
  return `
    <div class="col-6 col-md-4 col-lg-3 mb-3">
      <div class="product-card-sm position-relative" onclick="window.location.href='${productLink}'" style="cursor: pointer;">
        ${discountPercent > 0 ? `<span class="product-discount-badge">خصم ${discountPercent}%</span>` : ''}
        
        <div class="img-box">
          <img src="${image}" alt="${product.title}" class="no-open-tab" ondragstart="return false;">
        </div>
