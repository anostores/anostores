/* ==========================================================================
   ...
   Guaranteed Hybrid Local & Production Clean Routing Support.
   ========================================================================== */
  ...
  async function checkMaintenanceMode() {
    ...
    try {
      ...
      // استثناء صفحات الأدمن وصفحة الصيانة نفسها من التوجيه التلقائي
      ...
      if (data?.value?.is_enabled) {
        ...
        const targetMaintenanceUrl = isLocalServer ? "under-maintenance.html" : "/under-maintenance";
  ...
  /**
   * 0.2 جلب وتحديث أرقام الواتساب والدعم المباشر ديناميكياً
   */
  ...
  /**
   * 0.3 جلب وتحميل السلايدر الرئيسي الديناميكي من Supabase
   */
  async function loadDynamicHeroSlider() {
    ...
    try {
      ...
      sliderInner.innerHTML = slides.map((slide, index) => {
        ...
        if (!isLocalServer && slideLink.endsWith(".html")) {
  ...
  /**
   * 0.4 جلب منتجات الصفحة الرئيسية وتوزيعها حسب الأقسام
   */
